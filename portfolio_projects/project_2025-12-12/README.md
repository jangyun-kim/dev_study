...
Raw event log를 session 단위로 변환하고,
모델 학습 이전에 필요한 Feature Store를 구축한다.

## Why This Matters

실무에서는 모델보다
'어떤 단위의 데이터를 쓰느냐'가 성능을 좌우한다.
...
| Day   | Focus                             |
| ----- | --------------------------------- |
| Day 1 | Raw Event 이해 & Sessionization   |
| Day 2 | Session-level Text Aggregation    |
| Day 3 | Session-level Feature Engineering |
